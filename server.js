...
try {
  ({ resumeRoutes } = require("./src/api/resume.routes"));
  ({ dashboardRoutes } = require("./src/api/dashboard.routes"));
  ({ errorMiddleware } = require("./src/middlewares/error.middleware"));
...
// Middleware
app.use(cors({
  origin: ["http://localhost:3001", "http://127.0.0.1:3001", "http://frontend:3001"],
  credentials: true,
...
// Logging
if (pinoHttp && logger && logger.levels) {
